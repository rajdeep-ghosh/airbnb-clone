  ...
  return (
    ...
    >
      ...
      <div onClick={() => router.push("/")} className="!hidden md:!block">
        <svg className={`w-[110px] h-12 cursor-pointer ${page ? `text-airbnb_red` : (showNav ? `text-airbnb_red` : `text-white`)}`} xmlns="http://www.w3.org/2000/svg" viewBox="329.775 439.999 320.426 100.002" >
          <path
      ...
      <div className="relative flex flex-col items-center">
        <div className="flex justify-between items-center p-2 w-fit md:w-[40vw] max-w-3xl border-2 rounded-full sm:shadow-sm bg-white">
          <input
            ...
            required
          />
          <button onClick={search} className="btn rounded-full bg-airbnb_red text-white ring-airbnb_red">
      ...
      {/* Right */}
      <div className={`hidden sm:flex space-x-3 items-center ${page ? `text-zinc-500` : (showNav ? `text-zinc-500` : `text-white`)}`}>
