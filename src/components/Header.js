...
function Header({ page }) {
  ...
  return (
    <header
      className={`fixed top-0 flex justify-center sm:justify-between items-center py-4 px-6 sm:px-14 xl:px-40 w-full z-50 !font-Nunito-Sans transition-all duration-200 ${page === "search" ? `bg-white shadow-md` : (showNav && `bg-white shadow-md`)}`}
    >
      {/* Left */}
      <div className="!hidden md:!block">
        <svg className={`w-[110px] h-12 cursor-pointer ${page === "search" ? `text-airbnb_red` : (showNav ? `text-airbnb_red` : `text-white`)}`} xmlns="http://www.w3.org/2000/svg" viewBox="329.775 439.999 320.426 100.002" >
          <path
      ...
      {/* Right */}
      <div className={`hidden sm:flex space-x-3 items-center ${page === "search" ? `text-zinc-500` : (showNav ? `text-zinc-500` : `text-white`)}`}>
        <p className="hidden md:inline text-sm cursor-pointer">Become a host</p>
        <GlobeAltIcon className="h-5 cursor-pointer" />
        <div className="flex items-center space-x-2 p-2 border-2 rounded-full bg-white text-zinc-500">
