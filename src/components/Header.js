...
function Header() {
  ...
  return (
    <header
      className={`fixed top-0 flex justify-center sm:justify-between items-center py-4 px-6 sm:px-14 xl:px-40 w-full z-50 !font-Nunito-Sans transition-all duration-200 ${showNav && `bg-white shadow-md`}`}
    >
      {/* Left */}
      <div className="!hidden md:!block">
        <svg className={`w-[110px] h-12 text-white cursor-pointer ${showNav && `text-airbnb_red`}`} xmlns="http://www.w3.org/2000/svg" viewBox="329.775 439.999 320.426 100.002" >
          <path
      ...
      {/* Right */}
      <div className={`hidden sm:flex space-x-3 items-center text-white ${showNav && `text-zinc-500`}`}>
